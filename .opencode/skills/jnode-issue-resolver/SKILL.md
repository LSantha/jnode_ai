...
3. **Work** — read → patch → build (`sh build.sh cd-x86-lite`) → focused test (`cd <subproject> && ant test`).
   - **Investigation budget.** Open-ended investigation requests (e.g. "why does X behave like Y?", "can you take a look?") are a trap: you can read files for 30 min and never reach a conclusion. **Limit yourself to 10–15 file reads or LLM steps before posting a partial report** — even if confidence is low. A partial report is better than silence; the human can iterate.
